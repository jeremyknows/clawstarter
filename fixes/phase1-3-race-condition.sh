#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
# OpenClaw Quickstart v2.3.1 — SECURITY FIX: Race Condition Patch
#
# FIXED: All sensitive file writes now use secure permissions from creation
# No window where API keys or tokens are world-readable
#
# Changes from v2.3.0:
# - Config file: umask 077 before Python write
# - LaunchAgent plist: Created with 600 before content write
# - AGENTS.md: Created with 600 for workspace files
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
# ═══════════════════════════════════════════════════════════════════
# SECURITY HELPER: Create file with secure permissions atomically
# ═══════════════════════════════════════════════════════════════════

secure_write() {
    local filepath="$1"
    local content="$2"
    local mode="${3:-600}"  # Default to 600
    ...
    # Method 1: Use umask for atomic secure creation
    (
        umask 077  # Ensures file created with 600
        echo "$content" > "$filepath"
    )
    
    # Method 2: Explicit chmod for specified permissions
    chmod "$mode" "$filepath"
}
...
offer_skill_packs() {
    ...
    if [ ! -f "$workspace_dir/AGENTS.md" ]; then
        ...
        # SECURITY FIX: Create with secure permissions first
...
# ═══════════════════════════════════════════════════════════════════
# STEP 3: Start Bot — SECURITY FIXES APPLIED HERE
# ═══════════════════════════════════════════════════════════════════
...
step3_start() {
    ...
    # ═══════════════════════════════════════════════════════════════
    # SECURITY FIX: Create config file with secure permissions FIRST
    # ═══════════════════════════════════════════════════════════════
    
    # Ensure directory exists
    mkdir -p "$(dirname "$config_file")"
    
    # Create file with 600 permissions BEFORE writing secrets
    touch "$config_file"
    chmod 600 "$config_file"
    
    # Now generate config with umask protection
    (
        umask 077  # Ensures any temp files are also secure
...
# File already exists with 600 perms, safe to write
...
PYEOF
    ...
    # Verify permissions
    chmod 600 "$config_file"
    pass "Config created (secure)"
    
    # ═══════════════════════════════════════════════════════════════
    # SECURITY FIX: Create workspace files with secure permissions
    # ═══════════════════════════════════════════════════════════════
    
    mkdir -p "$workspace_dir/memory"
    
    # Create AGENTS.md with secure permissions first
    touch "$workspace_dir/AGENTS.md"
    chmod 600 "$workspace_dir/AGENTS.md"
...
AGENTSEOF
    ...
    if [ -n "$QUICKSTART_TEMPLATES" ]; then
        ...
        for template_name in "${TEMPLATE_ARRAY[@]}"; do
            ...
            if $first_template; then
                ...
                # Create temp file with secure perms, then move
                local temp_file
                temp_file=$(mktemp)
                chmod 600 "$temp_file"
                
                if curl -fsSL "$agents_url" -o "$temp_file" 2>/dev/null; then
                    mv "$temp_file" "$workspace_dir/AGENTS.md"
                    chmod 600 "$workspace_dir/AGENTS.md"
    ...
    # ═══════════════════════════════════════════════════════════════
    # SECURITY FIX: Create LaunchAgent plist with secure permissions
    # ═══════════════════════════════════════════════════════════════
    
    local launch_agent="$HOME/Library/LaunchAgents/ai.openclaw.gateway.plist"
    mkdir -p "$HOME/Library/LaunchAgents"
    
    # Create file with secure permissions FIRST
    touch "$launch_agent"
    chmod 600 "$launch_agent"
    
    # Now write content (file already has secure perms)
...
main() {
    ...
    echo -e "${BOLD}  3 Questions → Running Bot (SECURE)${NC}"
    echo -e "${BOLD}═══════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo -e "  ${INFO} No race windows — files created with 600 from start"
    echo -e "  ${INFO} API keys and tokens never world-readable"
    echo ""
