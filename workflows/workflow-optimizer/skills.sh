...
mkdir -p "$WORKSPACE/weekly"
echo "  ✓ Created inbox/, calendar/, tasks/, daily/, weekly/"

# Verify installations
echo ""
echo -e "${CYAN}Verifying installations...${NC}"
command -v memo >/dev/null && echo "  ✅ Apple Notes (memo)" || echo "  ❌ memo FAILED"
command -v remindctl >/dev/null && echo "  ✅ Apple Reminders (remindctl)" || echo "  ❌ remindctl FAILED"
command -v gog >/dev/null && echo "  ✅ Google Workspace (gog)" || echo "  ❌ gog FAILED"
command -v himalaya >/dev/null && echo "  ✅ Email (himalaya)" || echo "  ❌ himalaya FAILED"
